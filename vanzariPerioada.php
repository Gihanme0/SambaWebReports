...
$startInput = isset($_POST['startDate']) ? trim($_POST['startDate']) : '';
$endInput = isset($_POST['endDate']) ? trim($_POST['endDate']) : '';
$hasReport = ($startInput !== '' && $endInput !== '');
$startTs = $hasReport ? strtotime($startInput) : false;
$endTs = $hasReport ? strtotime($endInput) : false;
...
if ($hasReport && $startTs && $endTs) {
    if ($endTs < $startTs) { $endTs = $startTs; }
    $displayStart = date('Y-m-d', $startTs);
    $displayEnd = date('Y-m-d', $endTs);
    $periodStartSql = date('Y-m-d', $startTs) . "T06:00:00.000";
    $periodEndSql = date('Y-m-d', strtotime('+1 day', $endTs)) . "T06:00:00.000";
...
<body class="PaginaVanzari kx-page kx-periodic-sales-page">
...
<main class="kx-shell">
    <section class="kx-hero">
        <div>
            ...
            <p>Periodic sales dashboard with items, payments, tickets, service charge, discounts and hourly sales.</p>
        </div>
    ...
    <section class="kx-panel kx-filter-panel">
        <form action="vanzariPerioada.php" method="post" role="form" id="kxReportForm">
            <div class="kx-filter-grid kx-range-filter-grid">
                <div class="kx-field">
                    <label>Start Date</label>
                    <div class="input-group date form_datetime_raport1 kx-date-picker" data-date-format="dd-mm-yyyy">
                        <input class="form-control" size="16" type="text" placeholder="Start date" name="startDate" value="<?php echo kx_h($startInput); ?>" readonly>
                ...
                <div class="kx-field">
                    ...
                    <div class="input-group date form_datetime_raport1 kx-date-picker" data-date-format="dd-mm-yyyy">
                        <input class="form-control" size="16" type="text" placeholder="End date" name="endDate" value="<?php echo kx_h($endInput); ?>" readonly>
                        <span class="input-group-addon"><span class="glyphicon glyphicon-remove"></span></span>
                    </div>
                </div>
                <div class="kx-actions">
                    ...
                    <?php if ($hasReport) { ?>
                        <button type="button" class="kx-btn kx-btn-success" onclick="kxExportExcel('complete_sales_export','Periodic_Sales_Report')">Excel</button>
                        <button type="button" class="kx-btn kx-btn-danger" onclick="kxExportPdf('Periodic Sales Report','Periodic_Sales_Report')">PDF</button>
    ...
    <?php if ($hasReport && $startTs && $endTs) { ?>
        <section class="kx-alert">
            <span class="kx-dot"></span>
            Report period: <strong><?php echo kx_h($displayStart); ?> 06:00</strong> to <strong><?php echo kx_h(date('Y-m-d', strtotime('+1 day', $endTs))); ?> 06:00</strong>
        </section>
        ...
        <div id="complete_sales_export" style="display:none">
            ...
            <p><?php echo kx_h($displayStart); ?> 06:00 to <?php echo kx_h(date('Y-m-d', strtotime('+1 day', $endTs))); ?> 06:00</p>
    ...
    <?php } else { ?>
        <section class="kx-empty-state"><div class="kx-empty-icon">📊</div><h2>Select start and end date</h2><p>Periodic sales report will show items, payments, tickets, service charge, discounts, charts and Excel export.</p></section>
    <?php } ?>
</main>
